...
function Validate-ParametersSQLSuite {
    param (
...
#Validation of DB Engine parameteres
...
#Validation of SSAS Parameters
     if (-not $Parameters["ASCollation"]) { Write-Error "Error: AS Collation is missing.";return $null   }

    # Directory path validations
    $ASdirectories = @(
        @{Name = "ASDataDir"; Value = $Parameters["ASDataDir"]},
        @{Name = "ASLogDir"; Value= $Parameters["ASLogDir"]},
        @{Name = "ASBackupDir"; Value=$Parameters["ASBackupDir"]},
        @{Name = "ASTempDir"; Value=$Parameters["ASTempDir"]},
        @{Name = "ASConfigDir"; Value=$Parameters["ASConfigDir"]}
        )
    foreach ($dir in $ASdirectories) {
        if (-not $dir.Value) {
            Write-Error "Error: $($dir.Name) is missing."; return $null 
        }
    }
    
    if (-not $Parameters["ASServerMode"]) { Write-Error "Error:ASServerMode Value is missing.";return $null   }
    if (-not $Parameters["ASProviderMode"]) { Write-Error "Error:ASProviderMode Value is missing.";return $null   }

    if (-not $Parameters["ASSYSAdmAccounts"]) {Write-Error "Error: AS Sysadmin Accounts are missing.Atleast Once account should have SysAdmin access."; return $null}
    
    if ($Parameters["ASServiceStartType"] -notin $validStartTypes) {
        Write-Error "Error: AS Service Start Type must be one of: Automatic, Manual, Disabled.";return $null  
    }

#Validation of SSRS Parameters    
    if (-not $Parameters["RSInstallType"]) { Write-Error "Error:RSInstallType Value is missing.";return $null   }  

    if ($Parameters["RSServiceStartType"] -notin $validStartTypes) {
        Write-Error "Error: RS Service Start Type must be one of: Automatic, Manual, Disabled.";return $null  
    }

#Validation of SSIS Parameters
    if ($Parameters["ISServiceStartType"] -notin $validStartTypes) {
        Write-Error "Error: IS Service Start Type must be one of: Automatic, Manual, Disabled.";return $null  
    }
...
#Validate-ParametersSQLSuite 'C:\Users\PAM-Deepam.Ghosh\Desktop\SQLAutomate\SQLInstaller\Parameters\SQLSuite_Install_Parameters.txt'
